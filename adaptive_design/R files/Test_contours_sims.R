...
fun_wrap <- function(X, case, fixedData){
  if(is.null(nrow(X))){
  ...
  f <- cov_gen(X, fixedData$X0, theta = c(0.2, 0.2), type = "Gaussian") %*% (fixedData$Kmatinv %*% fixedData$Y0)
...
run_cont <- function(ii){
  ...
  for(i in 1:k)
  {
    ...
    if(ftype == "gp"){
      if(case[1] == "normal"){
        if(case[2] == "small") fixedData <- readMat("../examples/contour/tp_gp_data/normal_small/gp.mat") else fixedData <- readMat("../examples/contour/tp_gp_data/normal_large/gp.mat")
      }
      if(case[2] == "t_constdf"){
        if(case[2] == "small") fixedData <- readMat("../examples/contour/tp_gp_data/t_constdf_small/gp.mat") else fixedData <- readMat("../examples/contour/tp_gp_data/t_constdf_large/gp.mat")
      }
    }
    if(ftype == "tp"){
      if(case[1] == "normal"){
        if(case[2] == "small") fixedData <- readMat("../examples/contour/tp_gp_data/normal_small/tp.mat") else fixedData <- readMat("../examples/contour/tp_gp_data/normal_large/tp.mat")
      }
      if(case[2] == "t_constdf"){
        if(case[2] == "small") fixedData <- readMat("../examples/contour/tp_gp_data/t_constdf_small/tp.mat") else fixedData <- readMat("../examples/contour/tp_gp_data/t_constdf_large/tp.mat")
      }
    }
    
    ft <- cov_gen(xtt, fixedData$X0, theta = c(0.2, 0.2), type = "Gaussian") %*% (fixedData$Kmatinv %*% fixedData$Y0)
    
    
    X <- maximinESE_LHS(lhsDesign(n = I, dimension = d, seed = i)$design)$design
    Y <- fun_wrap(X = X, case = case, fixedData = fixedData)
    ...
    for(j in 1:(budget - I)){
      ...
      Xnew <- sol$par
      X <- rbind(X, Xnew); Ynew <- fun_wrap(X = Xnew, case = case, fixedData = fixedData)
      Y <- c(Y, Ynew)
  ...
  writeMat(paste0(class(model), "_", crit, "_", case[1], "_", case[2], "_", d, "d_simu.mat"),
...
### Some graphs
if(FALSE){
  data_res <- readMat("../examples/contour/Simu_results/homTP_crit_MCU_normal_small_2d_simu.mat")
